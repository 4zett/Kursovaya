...
class Game:
    def __init__(self, canvas: Canvas, x_field_size: int, y_field_size: int):
        ...
        self.checkers_type_for_game()
        self.__draw()
        # Если игрок играет за чёрных, то совершить ход противника
        if PLAYER_SIDE == SideType.BLACK:
            self.__handle_enemy_turn()

    def checkers_type_for_game(self):
        '''Инициализация изображений'''
        self.__images = {
            CheckerType.WHITE_REGULAR: 'white',
            CheckerType.BLACK_REGULAR: 'black',
        }
        ...
        for y in range(self.__field.y_size):
            for x in range(self.__field.x_size):
                ...
                if checker_type != CheckerType.NONE:
                    color = 'white' if checker_type == CheckerType.WHITE_REGULAR else 'black'
                    border_color = 'black' if color == 'white' else 'white'
                    border_width = 5  # Устанавливаем ширину границы
